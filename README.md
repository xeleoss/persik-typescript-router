##Приложение создано с помощью команды
 ```vkma app-name --template typescript```

##Используемая библиотека для роутинга 
https://github.com/HappySanta/router 
#
...
`yarn start` || `yarn start` — this will start dev server with hot reload on `localhost:10888`.

`yarn run build` || `yarn build` — this will build production bundle, with tree-shaking, uglify and all this modern fancy stuff
